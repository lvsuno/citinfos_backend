...
const MunicipalitySelector = ({
  ...
  // Gérer la sélection initiale avec le contexte
  useEffect(() => {
  ...
  const handleMunicipalitySelect = (municipality) => {
    ...
    switchMunicipality(municipality.nom);

    // Naviguer vers la nouvelle municipalité
    const slug = getMunicipalitySlug(municipality.nom);
          ...
          {municipalities.length > 0 ? (
            municipalities.map((municipality) => (
              <button
                ...
                type="button"
                className={`${styles.municipalityOption} ${selectedMunicipality?.nom === municipality.nom ? styles.selected : ''
                  }`}
