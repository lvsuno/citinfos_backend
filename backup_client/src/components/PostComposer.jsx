...
const PostComposer = ({ onSubmit, community = null, placeholder = "What's happening?", maxLength = 2000 }) => {
  ...
  const handleSubmit = async () => {
    ...
    if (visibility === 'custom' && !customFollowers.trim()) {
      ...
      alert('Please specify users or communities for custom visibility');
      return;
    }
      ...
      <div className={`p-4 ${isDragging ? 'opacity-40' : ''}`}>
        ...
        <div className="relative">
          <textarea
            ref={mentionInputRef}
            rows={2}
            placeholder="Share an update, ask a question, or drop files..."
        ...
        {showPoll && (
          <div className="mt-4 border border-indigo-200 rounded-md p-3 bg-indigo-50/40">
            <div className="flex items-center justify-between mb-2">
              <p className="text-xs font-semibold text-indigo-700 tracking-wide">Poll</p>
              <button type="button" onClick={()=>setShowPoll(false)} className="text-[11px] text-indigo-500 hover:text-indigo-700 font-medium">Remove</button>
            </div>
            <input
              type="text"
              value={pollQuestion}
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3 border-t pt-3 border-gray-100" ref={toolbarRef}>
            ...
            {showEmojiPicker && (
              ...
              >
                ...
                <div className="mt-2 px-2 pb-2 overflow-y-auto" role="grid">
                  ...
                  <p className="sr-only">Use arrow keys. Search by name. Hover to see description.</p>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="ml-auto flex items-center gap-2">
            <button type="button" onClick={reset} disabled={!(text || attachments.length || showPoll)} className="text-[11px] font-medium text-gray-500 hover:text-gray-700 disabled:opacity-40 disabled:cursor-not-allowed">Clear</button>
            <button type="button" onClick={handleSubmit} disabled={!canSubmit()} className="px-4 py-1.5 rounded-md bg-indigo-600 text-white text-sm font-semibold shadow-sm hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">{isSubmitting ? 'Posting...' : 'Post'}</button>
          </div>
        </div>
