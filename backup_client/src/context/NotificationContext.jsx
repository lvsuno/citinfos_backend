...
import api from '../services/axiosConfig'; // Add API import for fetching existing notifications
...
export const NotificationProvider = ({ children }) => {
  ...
  const fetchExistingNotifications = async () => {
    try {
      ...
      const response = await api.get('/notifications/?limit=50'); // Get last 50 notifications
  ...
  useEffect(() => {
    ...
    if (token && !jwtAuthService.isTokenExpired(token) && !state.isConnected && !state.isConnecting) {
  ...
  const actions = {
    ...
    reconnect: () => {
      const token = jwtAuthService.getAccessToken();
      if (token && !jwtAuthService.isTokenExpired(token)) {
        dispatch({
          type: ActionTypes.SET_CONNECTION_STATUS,
          payload: { connected: false, connecting: true, failed: false }
        });
