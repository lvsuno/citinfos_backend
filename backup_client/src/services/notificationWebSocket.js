...
class NotificationWebSocket {
  ...
  /**
   * Reconnect WebSocket with new JWT token (called from jwtAuthService)
   */
  ...
  hasValidAuthentication() {
    ...
    // Check session (optional but recommended)
    if (!sessionId) {
      console.warn('No session ID found in token');
      // Don't fail here, backend can handle sessionless connections
    }
  ...
  async attemptTokenRefreshAndReconnect() {
    try {
      ...
      const refreshed = await jwtAuthService.refreshAccessToken();
