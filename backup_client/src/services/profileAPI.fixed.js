...
class ProfileAPI extends BaseAPIService {
  ...
  async toggleFollowUser(userId) {
    try {
      console.log('🔄 toggleFollowUser called for userId:', userId);

      // Try the new backend toggle endpoint that handles soft-deleted records
      console.log('🚀 Calling backend toggle endpoint...');
      const result = await this.post('/follows/toggle/', { followed: userId });
      console.log('✅ Toggle successful:', result);
      return result;
    } catch (error) {
      console.log('⚠️ Toggle endpoint failed, using fallback...', error.response?.status);

      // Fallback to original approach if toggle endpoint doesn't exist or fails
      try {
        console.log('➕ Attempting to create new follow relationship...');
        const result = await this.followUser(userId);
        console.log('✅ Successfully created follow relationship:', result);
        return result;
      } catch (followError) {
        console.log('⚠️ Follow failed, checking if already following...', followError.response?.status);

        if (followError.response?.status === 400) {
          // Probably already following, so try to unfollow
          console.log('🔍 Getting existing follow relationships...');
          const follows = await this.get(`/follows/?followed=${userId}`);
          console.log('📋 Found follow relationships:', follows.results);

          if (follows.results && follows.results.length > 0) {
            const followRelation = follows.results[0];
            console.log('🗑️ Unfollowing (soft delete):', followRelation.id);
