            ...
            <div className={`${styles.mapPageContainer} ${isVisible ? styles.visible : ''}`}>
                ...
                <div className={styles.mainInterface}>
                    ...
                    <div className={styles.compactSidebar}>
                        ...
                        {/* Top cities compacte */}
                        {stats?.largestCities && (
                            <div className={styles.compactTopCities}>
                                <div className={styles.compactCardHeader}>
                                    <AutoAwesomeIcon className={styles.compactCardIcon} />
                                    <span className={styles.compactCardTitle}>Top 5 Villes</span>
                                </div>
                                <div className={styles.compactCitiesList}>
                                    {stats.largestCities.slice(0, 5).map((city, index) => (
                                        <div
                                            key={city.nom}
                                            className={styles.compactCityItem}
                                            onClick={() => handleSearchResultClick(city)}
                                        >
                                            <div className={styles.compactCityRank}>#{index + 1}</div>
                                            <div className={styles.compactCityInfo}>
                                                <span className={styles.compactCityName}>{city.nom}</span>
                                                <span className={styles.compactCityPop}>{Math.round(city.population / 1000)}k</span>
                                            </div>
                                        </div>
                                    ))}
                                </div>
