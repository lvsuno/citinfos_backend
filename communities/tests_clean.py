...
class CommunityModelTests(TestCase, JWTAuthTestMixin):
    ...
    def test_only_active_members_can_interact(self):
        ...
        banned_profile = UserProfile.objects.create(
            user=user3,
            bio="Banned user"
        )
        ...
        deleted_profile = UserProfile.objects.create(
            user=user4,
            bio="Deleted user"
        )
...
class CommunityIntegrationTests(TestCase, JWTAuthTestMixin):
    ...
    def setUp(self):
        ...
        self.applicant_profile = UserProfile.objects.create(
            user=self.user3,
            bio="Applicant user"
        )
    ...
    def test_community_membership_creation(self):
        ...
        self.assertIsNone(membership.ban_reason)
    ...
    def test_community_unique_constraints(self):
        """Test unique constraints on community models."""
        ...
        # Test unique membership per user per community
        ...
        # Test unique invitation per user per community
        expires_at = timezone.now() + timedelta(days=7)
        ...
        # Test unique join request per user per community
        ...
        with self.assertRaises(Exception):
            CommunityJoinRequest.objects.create(
                community=self.community,
                user=self.userprofile2  # Same user for same community
            )
